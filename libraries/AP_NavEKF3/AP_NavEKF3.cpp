...
{
    if (new_lane_index >= num_cores || new_lane_index == primary) {
        return;
    }

    const uint8_t oldPrimaryIndex = primary;
    alignLaneSwitchPositionIfNeeded(new_lane_index, primary);
    ...
    send_lane_switch_reason(
        ...
        core[new_lane_index],
        requested_forced_primary_core() == new_lane_index);
}
...
{
    if (configuredToUseGPSForPosXY() || sources.getPosZSource(uint8_t(get_active_source_set())) == AP_NavEKF_Source::SourceZ::GPS) {
        return true;
    }
    return false;
}
...
void NavEKF3::send_eagle_lane_states_report(GCS_MAVLINK &link) const
{
    if (!core) {
        return;
    }
